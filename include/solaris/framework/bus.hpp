...
template<typename C>
class Bus {
  std::unordered_map<std::type_index, std::unique_ptr<BaseDispatcher<C>>> m_Dispatchers{};
...
public:
  ...
  Dispatcher<E,C>& getDispatcherFor() {
    ...
    auto [it, inserted]{m_Dispatchers.insert({typeid(E), std::make_unique<Dispatcher<E, C>>()})};
    //BaseDispatcher<C>& dispatcher{*m_Dispatchers[typeid(E)] = std::make_unique<Dispatcher<E, C>>()};
    //auto [it, inserted]{m_Dispatchers.try_emplace(typeid(E))};
    return dynamic_cast<Dispatcher<E, C>&>(*it->second);
